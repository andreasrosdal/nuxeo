...
public class StreamServiceImpl extends DefaultComponent implements StreamService {
    ...
    protected void initProcessor(StreamProcessorDescriptor descriptor) {
        ...
        try {
            Topology topology = descriptor.klass.getDeclaredConstructor().newInstance().getTopology(descriptor.options);
            StreamProcessor streamProcessor = new LogStreamProcessor(manager);
            Settings settings = getSettings(Framework.getService(CodecService.class), descriptor);
    ...
    protected Settings getSettings(CodecService codecService, StreamProcessorDescriptor descriptor) {
